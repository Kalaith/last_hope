...
export const WorldMap = memo<WorldMapProps>(
  ({ gameState, className = '', onZoneClick }) => {
    ...
    const getZoneDescription = (state: MapZone['state']): string => {
      switch (state) {
        case 'dead':
          return 'Barren wasteland with poisoned soil';
        case 'sprouting':
          return 'First signs of life breaking through';
        case 'growing':
          return 'Visible vegetation taking hold';
        case 'thriving':
          return 'Lush growth and returning wildlife';
      }
    };
            ...
            content={
              <div>
                ...
                <div style={{ fontSize: '0.9em', whiteSpace: 'pre-line' }}>
                  This map shows your progress restoring the wasteland around
                  your base camp. Each zone represents a different area of the
                  ecosystem. As soil health improves, vegetation spreads outward
                  from your central restoration efforts. Click on zones to see
                  detailed information about their current state and what's
                  needed for further restoration.
                </div>
              </div>
            }
            position="bottom"
        ...
        <div className="map-status">
          ...
          >
            <div className="status-item" style={{ cursor: 'help' }}>
              ...
              <span className="value">
                {(gameState.ecosystem?.soilHealth || 0).toFixed(1)}%
              </span>
            </div>
          </Tooltip>
          <Tooltip
          ...
          >
            <div className="status-item" style={{ cursor: 'help' }}>
              ...
              <span className="value">
                {gameState.ecosystem?.plantDiversity || 0}
              </span>
            </div>
          </Tooltip>
          <Tooltip
          ...
          >
            <div className="status-item" style={{ cursor: 'help' }}>
              ...
              <span className="value">
                {gameState.ecosystem?.plantInstances?.length || 0}
              </span>
            </div>
          </Tooltip>
        </div>

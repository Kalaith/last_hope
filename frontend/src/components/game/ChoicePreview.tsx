  ...
  return (
    <div className="choice-preview">
      <div className="preview-header">
        ...
        <div className={`risk-indicator risk-${riskLevel}`}>Risk: {riskLevel.toUpperCase()}</div>
      </div>
        ...
        <div className="relationship-projections">
          ...
          {Object.entries(relationshipChanges).map(([npc, change], index) => (
            <div key={index} className="relationship-item">
              <span className="npc-name">{npc.charAt(0).toUpperCase() + npc.slice(1)}</span>
              <span className="trust-change" style={{ color: getChangeColor(change) }}>
                {change > 0 ? '👥 +' : '💔 '}
      ...
      <div className="preview-footer">
        <div className="preview-tip">💡 Consider how this fits your long-term strategy</div>
      </div>
    </div>
  );
});
